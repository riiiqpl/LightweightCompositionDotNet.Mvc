...
namespace LightweightCompositionDotNet.Mvc
{
    ...
    public static class CompositionProvider
    {
        ...
        {
            GlobalConfiguration.Configuration.DependencyResolver = new CompositionScopeHttpDependencyResolver();
        }
        ...
        private static void ConfigureMvc()
        {
            if (DependencyResolver.Current.GetType().Name != "DefaultDependencyResolver")
                throw new CompositionFailedException(
                    "The Composition Provider for ASP.NET MVC sets the application-wide DependencyResolver. It seems that another dependency resolver has already been set.");

            DependencyResolver.SetResolver(new CompositionScopeDependencyResolver());

            FilterProviders.Providers.Remove(
                FilterProviders.Providers.OfType<FilterAttributeFilterProvider>().SingleOrDefault());
